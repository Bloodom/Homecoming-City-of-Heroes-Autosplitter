// Works on Homecoming version 27.4.5076, Beta version 27.4.5087, Cryptic version 27.4.4868.
...
{
    int MissionSelected: 0xBD2770; // Navigation Status Window. MissionSelected should be 1068641 on Return to Contact, 16777216 on no mission, and 269502050 when mission is complete but the player is still in the mission (as well as other niche cases).
    int TeamLock: 0xBD4C4A; // Team is locked/unlocked. TeamLock value (4 Byte) should be 2042527 (or higher) when unlocked, 2031616 when TF started, 0 on loadscreen (10911 if team not locked and loading INTO SG base), 2031616 in mission or in base (regardless of TF status). TeamLock value (2 Byte) should be 10911 (or higher) when unlocked, 0 when TF started, 0 on loadscreen (10911 if team not locked and loading INTO SG base), 0 in mission or in base (regardless of TF status). Best bet to find is to set 2 Byte and then keep checking for changed values starting/quitting TFs and check for 0 values, since the 5-digit value changes daily. 
    int Zone: 0x86C2F4; // Different values depending on the type of instance the player is in. The value is 1 when in an Overworld zone, 5 when in a Supergroup base, and varies when in a mission instance (usually 0 or 4).
    int PopUp: 0x994078; // TF/SF/Trial completion pop-up. This address value will be 0 upon loading into a zone with no pop-up, < 400 when a mission pop-up, teleport prompt, hide prompt, or TT prompt appears, and > 400 on TF/SF/Trial complete pop-up. If re-finding this address, the value is often 316 on quit prompt.
}

state("cityofheroes", "Beta")
{
    int MissionSelected: 0xBD2770;
    int TeamLock: 0xBD4C4A;
    int Zone: 0x86C2F4;
    int PopUp: 0x994078;
}
...
init
{
    ...
    {
        case 23437312: version = "Homecoming";
        break;
        case 1: version = "Beta";
        break;
